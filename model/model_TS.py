...
class TS(nn.Module):
    ...
    #     # x = nn.MaxPool2d(kernel_size=(1, f))
    #     # print("x.spatial",x.shape)
    #     x = rearrange(x, '(b p) c f-> b p (c f)', p=p)
    #     # print("x.spatial",x.shape)
    #     return x
    #
    # def Spatial_forward_features(self, x):
    #     # print("s",x.shape)   # 32 22 256
    #     b = x.shape[0]
    #     n = x.shape[1]
    ...
    #         x = blk(x)
    #
    #     # print("x1", x.shape) ###32 22 512
    #
    #     b, p, c = x.shape
    #
    #     x = rearrange(x, 'b  p  c  -> b  c  p', )
    #     x = F.avg_pool2d(x, kernel_size=(1, p))
    #     # x = F.max_pool2d(x, kernel_size=(1, p))
    #     # print("x_mid", x.shape)   #32 512 1
    #
    #     x = x.mean(dim=2)
    #     # print("x_mid", x)
    #     # print("x_mid", x.shape)
    ...
    def Temporal_forward_features(self, x):
        ...
        # x = F.avg_pool2d(x, kernel_size=(1, f))
        new_f = int(f/2)
        x = F.max_pool2d(x, kernel_size=(1, new_f))
        # x = nn.MaxPool2d(kernel_size=(1, f))
        # print("x.spatial",x.shape)
        x = rearrange(x, '(b p) c f-> b p c f', p=p)
